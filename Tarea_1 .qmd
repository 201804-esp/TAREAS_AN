...
Consideremos $f(x)=4x^2-16x-21$

**a)Gráfica del polinomio:**

```{python}
#| code-fold: true
#| fig-align: "center"

f = lambda x: 4*x**2-8*x-14

x = np.linspace(-1.5, 3.5, 600)
y = f(x)
...
**b)Aproximaciones para la primera raíz**
...
Método Newton-Raphson. Hay que brindar la primera derivada $f'(x)= 8x-8$.

```{python}
#| code-fold: true

# Se brinda la derivada de la función
f_der = lambda x: 8*x - 8
...
root_secante, info_secante = optimize.newton(f, x0 =1.5, x1=3.5, full_output = True)
...
**Interacciones de las raices en cada metodo**

#Primera raíz: Bisección(-1.121320343559546), Newton(-1.1213203435596426), Secante(-1.1213203435596282)

#Segunda raíz: Bisección(3.121320343559546), Newton(3.121320343559643), Secante(3.1213203435596424)
...
Consideremos $f(x)= x^3-9x^2+16x-9$, 
...
root_biseccion, info_biseccion = optimize.bisect(f, -2, 7, full_output = True)
...
Método Newton-Raphson. Primera derivada $f'(x)= 3x^2-18x+16$.
...
root_secante, info_secante = optimize.newton(f, x0 = -2, x1= 7, full_output = True)
...
root_biseccion, info_biseccion = optimize.bisect(f, 3.75, 7, full_output = True)
...
root_newton, info_newton = optimize.newton(f, 3.75, f_der, full_output = True)
...
# Se aplica el método de la secante
root_secante, info_secante = optimize.newton(f, x0 = 3.75, x1= 7, full_output = True)
...
#Primera raíz: Bisección(6.858442553291866), Newton(6.858442562384561), Secante(6.858442553291625)

#Segunda raíz: Bisección(6.8584425532919795), Newton(6.858442553291622), Secante(6.858442553291623)

#Tercera ráiz: Bisección(6.858442553292775), Newton(6.8584425532916224), Secante(6.85844255329162)
:::
...
#Primera raíz: Bisección(No aplica), Newton(0.5671432790364022), Secante(0.5671433067942667)
:::
...
plt.figure(figsize=(8,5))  # Tamaño en pulgadas
plt.plot(x, y1, color="purple", label="np.exp(x) - 1")
plt.plot(x, y2, color="deeppink", label="np.exp(x) - 3")
plt.plot(x, y3, color="red", label="np.exp(x) - 5")
...
Método Newton-Raphson. La primera derivada $g'(x)=e^x$.
...
plt.plot(x,y1, color = "darkorange", label = " e^2 - log(x+1) - 2")
plt.plot(x,y2, color = "magenta", label = " e^2 - log(x+1) - 3")
...
#| code-fold: true
#| 
root_biseccion, info_biseccion = optimize.bisect(g2, -0.99, 1.5, xtol=2e-12, full_output = True)
...
for i in range(1, 20):
    ...
    fb = g2(b)
    if fa * fb < 0:
        print(f"Cambio de signo entre [{a:.1f}, {b:.1f}]")
        print(f"g2({a:.1f}) = {fa}, g2({b:.1f}) = {fb}")

root_biseccion, info_biseccion = optimize.bisect(g2, a, b, xtol=1e-12, full_output = True)
...
root_newton, info_newton = optimize.newton(g2, 2.0, g_der, tol=1e-12, full_output = True)
...
root_secante, info_secante = optimize.newton(g2, x0 = 2, x1= 1, tol=2e-12, full_output = True)
...
root_biseccion, info_biseccion = optimize.bisect(g3, 3, 0.5, xtol=2e-12, full_output = True)
...
root_newton, info_newton = optimize.newton(g3, 2, g_der, tol=1e-12, full_output = True)
...
root_secante, info_secante = optimize.newton(g3, x0 = 2, x1= 0.5, full_output = True)
...
#Primer Raíz cuando b=2: Bisección(1.2071153719379395), Newton(-0.9976625606919567), Secante(1.2071153719371968)

#Segunda Raíz cuando b=2: Bisección(2.0), Newton(2.0), Secante(2.0)

#Raíz cuando b=3: Bisección(1.6919515157512706), Newton(1.6919515157519889), Secante(1.6919515157519953)

#Raíz cuando b=5: Bisección(1.609437912435169), Newton(1.6094379124340807), Secante(1.6094379124341005)
:::
...
plt.plot(solucion_1[0], solucion_1[1], marker = 'o', ms=8, color= "darkgreen")
...
**a)Realiza una gráfica de las curvas de nivel**
...
plt.plot(solucion_1[0], solucion_1[1], marker = 'o', ms=8, color= "cyan")
plt.axhline(0, color = "black", linewidth = 0.8, linestyle = "--") #Eje x
plt.axvline(0, color = "black", linewidth = 0.8, linestyle = "--") #Eje y
...
plt.plot(solucion_1[0], solucion_1[1], marker = 'o', ms=8, color= "deeppink")
plt.axhline(0, color = "black", linewidth = 0.8, linestyle = "--") #Eje x
plt.axvline(0, color = "black", linewidth = 0.8, linestyle = "--") #Eje y
